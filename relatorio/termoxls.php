...
error_reporting(E_ALL);
 ...
 $sql= "Select termolabo.sku as SKU, sum(vendas.qtd) as VENDA_ACUMULADA,  Products.reference_code as EAN, Products.title as NOME, 
...
marca.marca as MARCA, marca.fabricante as FABRICANTE, Products.otc as OTC, Products.descontinuado as DESCONTINUADO, 
 Products.controlled_substance as CONTROLADO, Products.active as ATIVO from termolabo inner join Products on Products.sku=termolabo.sku inner join vendas on
vendas.sku=termolabo.sku
...
INNER JOIN principio_ativo ON principio_ativo.sku = Products.sku INNER JOIN descontinuado on Products.sku = descontinuado.sku 
 INNER JOIN marca on Products.sku = marca.sku group by sku";
$stmt = $pdo->query($sql);
...
//browser to download.
$filename = 'Total_Produtos_termolabil.xls';
