...
Products.curve as CURVA, pbm.active as PBM_ATIVO, descontinuado.situation as SITUACAO_DESCONTINUADO,
...
INNER JOIN principio_ativo ON principio_ativo.sku = Products.sku INNER JOIN descontinuado on Products.sku = descontinuado.sku 
 INNER JOIN marca on Products.sku = marca.sku inner join pbm on Products.sku=pbm.sku where Products.pbm=1 group by sku";
$stmt = $pdo->query($sql);
...
//browser to download.
$filename = 'Total_Produtos_PBM.xls';
